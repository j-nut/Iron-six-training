...
(() => {
  ...
  async function start(id){
    ...
    try{
      ...
      const options={redirectTo:window.IronSixNative?.redirectTo||'https://iron-six-training.vercel.app/?auth=oauth'};
      ...
      const messages={
        local_save_failed:'Your latest changes could not be saved on this device. Your workout is still here; try again after the save completes.',
        manual_linking_disabled:'Connecting another provider is not available right now. Your current account is unchanged.',
        identity_already_exists:'That sign-in method is already connected to another Iron Six account.',
        provider_disabled:'This sign-in method is not enabled yet.',
        provider_not_enabled:'This sign-in method is not enabled yet.'
      };
      api.notify(messages[error.code]||'Could not open '+provider.name+'. Try again or continue with email.');
  ...
  function callbackError(){
    ...
    if(!code)return null;
    return {message:code==='access_denied'?'Sign-in was cancelled. You can try again or use email.':'The sign-in could not be completed. Please try again.',
  ...
  function install(options){
    ...
    for(const provider of providers){
      const button=document.createElement('button');button.id='social-'+provider.id;button.type='button';button.className='social-provider';
      button.innerHTML='<span class="social-icon">'+provider.icon+'</span><span class="social-label">Continue with '+provider.name+'</span><span class="social-chevron" aria-hidden="true">›</span>';
      button.onclick=()=>start(provider.id);$('socialProviders').append(button)
    }
