(() => {
  ...
  function runtimeState(u){return {today:u.today||{},history:u.history||[],coachMessages:u.coachMessages||[],coachOverrides:u.coachOverrides||null,localUpdatedAt:Number(u.localUpdatedAt)||Date.now()}}
  ...
  function fromRow(r){const s=r.runtime_state||{};return normalizeUser({id:r.client_id,name:r.display_name,weight:Number(r.body_weight_lb)||213,age:r.age||null,heightIn:r.height_in||null,trainingLevel:r.training_level||'unknown',benchBest:r.bench_reference||'',equipment:r.equipment||{},capacities:r.capacities||{},workoutMinutes:r.workout_minutes||60,readiness:r.readiness||{},program:r.program_state||{},today:s.today||{},history:s.history||[],coachMessages:s.coachMessages||[],coachOverrides:s.coachOverrides||null,localUpdatedAt:Number(s.localUpdatedAt)||Date.parse(r.updated_at)||0,cloudId:r.id})}
  ...
  async function pullAndMerge(){
    ...
    for(const r of rows){const cloud=fromRow(r),local=localById.get(String(r.client_id));if(!local){data.users.push(cloud);continue}const localTs=Number(local.localUpdatedAt)||0,cloudTs=Number(cloud.localUpdatedAt)||Date.parse(r.updated_at)||0;if(cloudTs>localTs)Object.assign(local,cloud)}
