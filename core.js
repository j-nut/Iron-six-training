...
function loadData(){
  ...
  const legacyRaw=window.ironSixAccountScope?null:readLocal(LEGACY_KEY);let jordan=makeUser(window.ironSixAccountScope?'My profile':'Jordan');if(!window.ironSixAccountScope)jordan.equipment.landmine=true;
...
function choose(u,options){const original=Array.isArray(options)?options:[],pool=[...original,...generatedOptionsForSlot(u,original)].filter((o,index,all)=>o?.name&&exerciseAvailable(u,o)&&all.findIndex(x=>x.name.toLowerCase()===o.name.toLowerCase())===index);if(!pool.length)return original.length?{...original[original.length-1]}:null;const cacheKey=currentSelectionKey(u,pool[0].base),cachedName=u.program?.selectionCache?.[cacheKey],cached=pool.find(x=>x.name===cachedName);const usesEquipment=o=>!!(o.requires?.length||o.requiresCustom?.length),equipped=pool.some(usesEquipment);
// Bodyweight entries are authored as last-resort fallbacks, but choose() ranks by position
// within the *filtered* pool, so a fallback becomes index 0 as soon as it is the only survivor.
// Without this, a user who owns one piece of equipment gets handed unloaded work instead of it.
// The penalty is smaller than an exact-name repeat (120), so recency still rotates through.
const fallbackPenalty=o=>equipped&&!usesEquipment(o)?24:0;
const ranked=cached||pool.map((o,index)=>({o,score:exerciseRecentPenalty(u,o)+index*1.5+(o.source==='groq'?2:0)+fallbackPenalty(o),tie:stableNumber(`${cacheKey}:${o.name}`)})).sort((a,b)=>a.score-b.score||a.tie-b.tie)[0].o;u.program.selectionCache=u.program.selectionCache||{};u.program.selectionCache[cacheKey]=ranked.name;const alternatives=pool.filter(x=>x.name!==ranked.name).map(x=>({...x}));return {...ranked,_alternatives:alternatives}}
...
function swapOptionsForExercise(u,exercise){const options=[...(exercise?._alternatives||[]),...generatedOptionsForSlot(u,[exercise])].filter((o,index,all)=>o?.name&&o.name!==exercise?.name&&o.seedKey===exercise?.seedKey&&exerciseAvailable(u,o)&&(u.trainingMode!=='circuit'||typeof circuitSuitable!=='function'||circuitSuitable(o))&&all.findIndex(x=>x.name.toLowerCase()===o.name.toLowerCase())===index);return options.sort((a,b)=>exerciseRecentPenalty(u,a)-exerciseRecentPenalty(u,b)||String(a.name).localeCompare(String(b.name))).slice(0,8)}
