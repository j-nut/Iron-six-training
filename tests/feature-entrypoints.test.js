...
for (const runtime of ['coach-recovery.js','auth-hardening.js','adaptive-insights.js','trainer-intelligence-v2.js']) assert(cloudHistory.includes(runtime),`runtime loader must request ${runtime}`);
for (const builder of ['scripts/build-web.mjs','scripts/build-android-web.mjs']) {
  const source=fs.readFileSync(builder,'utf8');
  for (const runtime of ['coach-recovery.js','auth-hardening.js','adaptive-insights.js','trainer-intelligence-v2.js']) assert(source.includes(runtime),`${builder} must package ${runtime}`);
}
