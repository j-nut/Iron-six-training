...
test('the spike ships no camera or upload path that runs without the flag',()=>{const source=fs.readFileSync('pose-spike.js','utf8');assert(source.indexOf("if(!readFlag())return")<source.indexOf('getUserMedia'));assert(!/fetch\(|XMLHttpRequest|supabase|\.upload\(/.test(source));assert(source.includes("categoryAllowlist:['person']"));assert(source.includes('trackingVersion:5'));for(const page of ['index.html','live.html']){const html=fs.readFileSync(page,'utf8');assert(html.includes('pose-rep-counter.js?v=2'));assert(html.indexOf('pose-rep-counter.js')<html.indexOf('pose-spike.js'))}});
