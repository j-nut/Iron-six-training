...
(() => {
  ...
  const RULES=[
    ...
    {id:'pushup',label:'Push-up',test:x=>/push[- ]?up/i.test(x.name||''),
     joint:[[P.LSHOULDER,P.LELBOW,P.LWRIST],[P.RSHOULDER,P.RELBOW,P.RWRIST]],
     framing:[P.LSHOULDER,P.RSHOULDER,P.LELBOW,P.RELBOW,P.LWRIST,P.RWRIST],
     top:155,bottom:100,minActiveMs:400,maxActiveMs:12000,
  ...
  // LOST_FRAMES catches a clean dropout. Real occlusion is not clean — it flickers, so a
  // consecutive-null counter alone never fires and the machine happily bridges a gap it was
  // blind through. QUALITY_WINDOW/MIN_QUALITY judge the recent past as a whole instead.
  const MIN_VISIBILITY=0.6, SMOOTHING=5, LOST_FRAMES=20, QUALITY_WINDOW=30, MIN_QUALITY=0.5, EDGE=0.02;
  ...
  function createCounter(rule,options){
    ...
    let samples=[],reps=[],armed=false,bottomed=false,repStart=0,extreme=null,lost=0,dropped=0,rejected=0,phase='waiting',angleNow=null,message='',recent=[];
    ...
    // True once the recent past is more blind than seeing. Only judged on a full window, so a
    // set never starts out "low quality".
    ...
    function push(frame){
      ...
      const usable=raw!==null&&frame&&frame.framed!==false;
      recent.push(usable?1:0);if(recent.length>QUALITY_WINDOW)recent.shift();
      if(!usable||qualityLow()){
        if(!usable){dropped++;lost++}
        // A brief occlusion mid-rep is normal and survivable. A sustained one — solid or
        // flickering — means the machine no longer knows where in the movement the lifter is,
        // so it re-arms from the top rather than counting across a gap it was blind through.
        if(lost>=LOST_FRAMES)rearm('Lost you. Step back into frame.');
      ...
      if(atTop(value)){
        // NOT the full rep: this is time spent below the top threshold, so it excludes the
        // travel between the standing angle and `top` at both ends — roughly 300ms of a normal
        // rep. The thresholds are named and tuned for what is actually measured, because
        // reading them as rep duration rejects an ordinary one-second cadence outright.
        const ms=t-repStart;
        ...
        if(ms>=config.minActiveMs&&ms<=config.maxActiveMs){reps.push({index:reps.length+1,ms,extreme:Math.round(extreme)});message=''}
        else{rejected++;message=ms<config.minActiveMs?'Skipped a bounce.':'Skipped a long pause.'}
