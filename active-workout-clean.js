/* Focused active-workout presentation: keep logging controls primary and move secondary coaching/detail UI behind deliberate taps. */
(() => {
  ...
  function addActions(card) {
    if (card.querySelector('.awc-actions')) return;
    const exercise = workoutFor(card);
    ...
    camera.addEventListener('click', () => {
      const original = card.querySelector('.pose-bar button');
      if (original) original.click();
    });

    const swap = actions.querySelector('.awc-swap-btn');
    swap.addEventListener('click', () => card.querySelector('.exercise-swap-btn')?.click());
  ...
  function apply() {
    ...
    cards.forEach(card => { addActions(card); updateActions(card); });
    const shown = cards.filter(visible);
    // Only simplify the one-exercise-at-a-time flow. "All exercises" remains the full planning view.
    list.classList.toggle('awc-focus', shown.length === 1);
  }

  const list = document.getElementById('exerciseList');
  if (list) new MutationObserver(() => queueMicrotask(apply)).observe(list, {childList:true,subtree:true,attributes:true,attributeFilter:['class']});
  document.addEventListener('click', event => {
    if (event.target.closest?.('#sessionCardNav,#sessionCardFoot,.done,.duration-btn,[data-view="today"]')) setTimeout(apply, 0);
  });
  setTimeout(apply, 0);
  addEventListener('load', apply);
  window.IronSixActiveWorkoutClean = { apply };
})();
