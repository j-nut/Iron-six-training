...
export default async function handler(req,res){
  ...
  try{
    const response=await fetch('https://api.groq.com/openai/v1/chat/completions',{method:'POST',headers:{Authorization:`Bearer ${process.env.GROQ_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model:MODEL,messages:[{role:'system',content:system},{role:'user',content:JSON.stringify({deterministicFactor:deterministic,profile:req.body?.profile||{},workout,completed})}],temperature:.1,max_completion_tokens:220})});
    const body=await response.json();if(!response.ok)return res.status(200).json({factor:Number(deterministic.toFixed(3)),summary:'The validated live calculation adjusted the remaining workout; cloud refinement was unavailable for this set.',model:'validated-calculation'});
