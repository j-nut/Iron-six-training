...
function currentSessionCompleted(u,exIndex){const arr=[];for(const [k,s] of Object.entries(u.today||{})){const [exerciseIndex,setIndex]=k.split('-').map(Number);if(exerciseIndex===exIndex&&s.done)arr.push({...s,_setIndex:setIndex})}return arr.sort((a,b)=>a._setIndex-b._setIndex)}
function baselineLoadObject(u,ex){const prior=priorPerformance(u,ex),target=repTarget(ex),targetRir=u.trainingLevel==='advanced'?1.5:2;if(prior.e1s.length){const e1=median(prior.e1s.slice(0,6)),load=roundLoad(e1/(1+(target+targetRir)/30),ex,u);return {load,target,text:`${load} lb × about ${target}`,confidence:'Performance-based',detail:`Based on ${prior.source==='exact'?'this exercise':'the same movement pattern'} from prior sessions.`}}if(ex.seedKey==='bench'){const b=benchReferenceE1RM(u);if(b){const load=roundLoad(b/(1+(target+2)/30),ex,u);return {load,target,text:`${load} lb × about ${target}`,confidence:'Benchmark-based',detail:'Seeded from the saved bench reference.'}}}const seed=demographicSeed(u,ex);if(seed)return {load:seed,target,text:`Start around ${seed} lb × ${target}`,confidence:'Conservative starting estimate',detail:'Uses body weight, age and training level only as a first-session estimate; your logged performance replaces it as soon as data exists.'};return {load:null,target,text:`Target ${target} controlled reps`,confidence:'Rep-based',detail:'Choose resistance that leaves about 2 reps in reserve.'}}
function calibratedSuggestion(u,ex,exIndex,base){const factor=Number(u.sessionCalibration?.factor)||1;if(currentSessionCompleted(u,exIndex).length||!base.load||Math.abs(factor-1)<.005)return base;const load=roundLoad(base.load*factor,ex,u),source=u.sessionCalibration?.source||'Live performance';return {...base,load,text:`${load} lb × about ${base.target}`,confidence:source,detail:u.sessionCalibration?.summary||'The remaining workout was recalibrated from today’s completed sets.'}}
function suggestedLoadObject(u,ex,exIndex){
 ...
 if(today.length){const recent=e1s.slice(0,6),e1=median(recent),load=roundLoad(e1/(1+(target+targetRir)/30),ex,u);return {load,target,text:`${load} lb × about ${target}`,confidence:'Updated from today',detail:'Adjusted from the weight, reps and RIR you entered in this workout.'}}
 return calibratedSuggestion(u,ex,exIndex,baselineLoadObject(u,ex));
}
function recalculateSessionCalibration(u,workout){const ratios=[];(workout||[]).forEach((ex,ei)=>{const base=baselineLoadObject(u,ex);if(!base.load)return;const targetRir=u.trainingLevel==='advanced'?1.5:2,expected=base.load*(1+(base.target+targetRir)/30);currentSessionCompleted(u,ei).forEach(set=>{const actual=e1rmFromSet(set);if(actual&&expected)ratios.push(actual/expected)})});if(!ratios.length){u.sessionCalibration=null;return null}const ratio=median(ratios),factor=Math.max(.94,Math.min(1.06,1+(ratio-1)*.35)),pct=Math.round((factor-1)*100);u.sessionCalibration={factor,source:'Live session',summary:pct===0?'Today’s completed sets support the planned loads.':`Today’s completed sets adjusted remaining loads ${pct>0?'+':''}${pct}%.`,updatedAt:Date.now()};return u.sessionCalibration}
function nextSetRecommendation(u,ex,exIndex){const sets=currentSessionCompleted(u,exIndex);if(!sets.length)return null;const last=sets[sets.length-1],w=parseLoad(last.weight),reps=Number(last.reps),rirText=String(last.rir??'').trim(),rir=rirText===''?null:Number(rirText),[lo,hi]=repRange(ex);if(!w||!reps)return null;let factor=1,label='Keep the same load';if(reps>=hi&&rir!==null&&rir>=2){factor=1.035;label='You beat the target — add a little weight'}else if(reps<lo||rir===0){factor=.94;label='That set was too hard — reduce the load'}else if(reps>=lo&&reps<=hi&&rir!==null&&rir>=1&&rir<=3){factor=1;label='Right on target — repeat this load'}else if(rir!==null&&rir>=4){factor=1.025;label='Too much left in reserve — add a little weight'}else if(rir===null){label=reps>hi?'Reps were high; add RIR next time for a sharper adjustment':'Repeat this load; add RIR for a sharper adjustment'}const load=roundLoad(w*factor,ex,u),target=Math.max(lo,Math.min(hi,repTarget(ex)));return {load,reps:target,label,text:`${label}: try about ${load} lb × ${target} next set.`}}
function nextSetAdjustment(u,ex,exIndex){return nextSetRecommendation(u,ex,exIndex)?.text||null}
