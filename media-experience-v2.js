(() => {
  ...
  function demo(name,media,compact){
    ...
    if(media.frames.length===1)return `<figure class="exercise-media ${compact?'compact':''}"><div class="motion-stage"><img style="opacity:1" src="${esc(media.frames[0])}" alt="${esc(name)} position" loading="lazy" decoding="async"></div><figcaption>${esc(name)} · ${esc(media.license||'licensed media')}</figcaption><div class="media-form-cues">${esc(cues(name))}</div></figure>`;
