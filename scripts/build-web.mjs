...
// Compact deployments retrieve the same committed originals. Verify every byte
// against the checked-in manifest before including an image in the release.
const pending = Object.entries(manifest.files);
await Promise.all(Array.from({length: 6}, async () => {
  while (pending.length) {
    const [file, expected] = pending.pop();
    if (!/^assets\/exercises\/[a-z0-9_-]+\.png$/.test(file) || !/^[a-f0-9]{64}$/.test(expected)) {
      throw Error('Invalid image manifest entry');
    }
    let bytes;
    try {
      ...
      const response = await fetch(`https://raw.githubusercontent.com/j-nut/Iron-six-training/${manifest.commit}/${file}`, {
