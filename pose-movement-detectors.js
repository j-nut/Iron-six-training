/* Iron Six movement-family camera rules.
   Extends the proven pose rep counter without changing its state machine. */
(() => {
  ...
  const FAMILIES = [
    {id:'squat',label:'Squat',patterns:['squat'],match:x=>/squat|lunge|split squat|step[- ]?up/i.test(x.name||''),joint:[[P.LHIP,P.LKNEE,P.LANKLE],[P.RHIP,P.RKNEE,P.RANKLE]],framing:[P.LSHOULDER,P.RSHOULDER,P.LHIP,P.RHIP,P.LKNEE,P.RKNEE,P.LANKLE,P.RANKLE],top:160,bottom:100,minActiveMs:500,maxActiveMs:15000,setup:'Place the phone around hip height, roughly level and side-on, far enough back to keep your full body in frame.',preferredView:'side',detectorFamily:'squat',formProfile:'squat',validated:true,fallback:{joint:[[P.LHIP,P.LKNEE],[P.RHIP,P.RKNEE]],framing:[P.LSHOULDER,P.RSHOULDER,P.LHIP,P.RHIP,P.LKNEE,P.RKNEE],top:30,bottom:75,note:'Ankles are out of frame, so reps are counted from your hips and knees. Depth is approximate and form watch is off.'}},
    {id:'hinge',label:'Hip hinge',patterns:['hinge'],match:x=>/deadlift|romanian|rdl|good morning|hip hinge/i.test(x.name||''),joint:[[P.LSHOULDER,P.LHIP,P.LKNEE],[P.RSHOULDER,P.RHIP,P.RKNEE]],framing:[P.LSHOULDER,P.RSHOULDER,P.LHIP,P.RHIP,P.LKNEE,P.RKNEE],top:160,bottom:105,minActiveMs:500,maxActiveMs:15000,setup:'Place the phone side-on around hip height. Keep shoulders, hips, and knees visible through the full hinge.',preferredView:'side',detectorFamily:'hinge',formProfile:'hinge',validated:false},
    {id:'horizontal_press',label:'Horizontal press',patterns:['bench','pushup','horizontal_press'],match:x=>/bench press|floor press|chest press|push[- ]?up/i.test(x.name||''),joint:[[P.LSHOULDER,P.LELBOW,P.LWRIST],[P.RSHOULDER,P.RELBOW,P.RWRIST]],framing:[P.LSHOULDER,P.RSHOULDER,P.LELBOW,P.RELBOW,P.LWRIST,P.RWRIST],top:155,bottom:90,minActiveMs:400,maxActiveMs:12000,setup:'Place the phone side-on near bench height so your shoulder, elbow, and wrist stay visible for the entire rep.',preferredView:'side',detectorFamily:'horizontal_press',formProfile:'horizontal_press',validated:false},
    {id:'vertical_press',label:'Vertical press',patterns:['overhead_press','vertical_press'],match:x=>/overhead press|shoulder press|military press|arnold press/i.test(x.name||''),joint:[[P.LSHOULDER,P.LELBOW,P.LWRIST],[P.RSHOULDER,P.RELBOW,P.RWRIST]],framing:[P.LSHOULDER,P.RSHOULDER,P.LELBOW,P.RELBOW,P.LWRIST,P.RWRIST],top:75,bottom:160,minActiveMs:450,maxActiveMs:12000,setup:'Face the phone or turn slightly side-on, far enough back that both hands remain visible overhead.',preferredView:'front-quarter',detectorFamily:'vertical_press',formProfile:'vertical_press',validated:false},
  ...
  function ruleFor(exercise) {
    ...
    return family ? {...family,sourcePattern:pattern||null,exerciseId:x.id||null,exerciseName:x.name||null} : originalRuleFor(exercise);
  }
  ...
  counter.ruleFor=ruleFor;counter.MOVEMENT_FAMILIES=FAMILIES;counter.familyFor=familyFor;counter.cameraSupported=supported;counter.cameraCoverage=coverage;counter.__ironSixMovementFamiliesV1=true;
